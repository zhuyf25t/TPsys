...
private[battle] object BattleFinishProjectionMailFactory {
  ...
  def battleMail(result: BattleResultRecord): IO[MailRecord] =
    replaySourcePath(result).map { sourcePath =>
      MailRecord(
  ...
  def ratingMail(result: BattleResultRecord): IO[MailRecord] =
    for
      ratingDelta <- signed(result.ratingDelta.value)
      sourcePath <- replaySourcePath(result)
    yield MailRecord(
      id = MailId(s"mail-rating-${result.resultId.value}"),
      ownerHandle = result.handle,
      kind = MailKind.Reward,
      subject = "Rating updated",
      excerpt = s"Rating $ratingDelta to ${result.ratingAfter.value}.",
  ...
  private def replaySourcePath(result: BattleResultRecord): IO[String] =
    for
      battleId <- urlEncode(result.battleId.value)
      handle <- urlEncode(result.handle.value)
    yield s"/replay/$battleId?handle=$handle"

  private def urlEncode(value: String): IO[String] =
    IO.pure(URLEncoder.encode(value, StandardCharsets.UTF_8))

  private def signed(value: Int): IO[String] =
    IO.pure(if value > 0 then s"+$value" else value.toString)
}
