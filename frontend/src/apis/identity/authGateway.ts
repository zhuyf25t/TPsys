...
import { normalizePlayableIdentityHandle } from "../../objects/identity/identityHandlePolicy";
import type {
  IdentityAuthResponseDto,
  IdentityRegistrationApiRequestDto,
  IdentitySessionApiRequestDto,
  SkinIdDto
...
type RemoteAuthPayload = IdentityAuthResponseDto;
...
async function tryRemoteRegister(input: {
  ...
  password: string;
  skinId: string;
} & IdentityRegistrationApiRequestDto): Promise<AuthResult | null> {
  try {
...
async function tryRemoteLogin(input: {
  handle: string;
  password: string;
} & IdentitySessionApiRequestDto): Promise<AuthResult | null> {
  try {
...
function normalizeRemoteAuthPayload(value: unknown): RemoteAuthPayload | null {
  ...
  const skinId = normalizeSkinId(readRequiredString(payload.skinId));
