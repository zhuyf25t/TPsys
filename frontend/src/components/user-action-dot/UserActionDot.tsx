...
import { recordContributionAdjustment, submitGovernanceReviewNotification } from "../../api/governance/governanceGateway";
import { getCurrentAuthUser, isBuiltinAdminHandle, subscribeAuthState } from "../../api/identity/authGateway";
import { getFriendRequestStatus, sendFriendRequest } from "../../api/social/friendRequestGateway";
import { buildBotProfilePath, isBotLikeHandle } from "../../objects/bots/botHandle";
  ...
  const panel = (
    ...
    >
      <div className="user-action-dot__header">
        ...
        <Link className="button-link" to={profilePath}>
          查看资料
        </Link>
      </div>
      ...
      {!isBotTarget ? (
        <section className="user-action-dot__section">
          ...
          <p className="user-action-dot__section-copy">{currentUser ? "发送好友申请后，对方会在站内收到通知。" : "登录后才能发送好友申请。"}</p>
          <button
            type="button"
      ...
      {isAdmin ? (
        <section className="user-action-dot__section user-action-dot__section--admin">
          ...
          <label className="user-action-dot__field">
            <span>原因说明</span>
            <textarea value={reason} onChange={(event) => setReason(event.target.value)} rows={3} maxLength={120} placeholder="可选补充说明。" />
          </label>

          <label className="user-action-dot__field">
            <span>自定义额度</span>
            <input value={customDelta} onChange={(event) => setCustomDelta(event.target.value)} inputMode="numeric" placeholder="可选整数。" />
          </label>

          {customDelta.trim() && (effectiveDelta === null || effectiveDelta === 0) ? <p className="user-action-dot__status">自定义额度必须是非 0 整数。</p> : null}
