...
1. 启动/验收脚本第二轮。
   预计：0.5-1 小时。
   目标：继续增强 dev 脚本的可读诊断，必要时增加只读验收摘要，不做自动杀进程。

2. 好友/社交抽屉接入真实 friend request 状态。
   预计：1-2 小时。
   目标：当前好友申请已有 gateway 和后端 service，但部分 UI 仍显示“暂无独立好友系统”之类旧文案；下一步先让社交入口准确反映好友申请状态，不急着做完整聊天。

3. 站内信第二轮。
   预计：1-2 小时。
   目标：继续压缩空白、统一战报+rating 通知的聚合展示策略，并补足旧消息兼容或清理策略。
...
下一票优先继续做“安全启动/验收脚本”或“社交入口真实状态”这类不会碰 BattlePage 素材、收益明确、风险可控的任务。大改功能代码继续按一票一 worker、一票一审查、一票一接受的方式推进。
