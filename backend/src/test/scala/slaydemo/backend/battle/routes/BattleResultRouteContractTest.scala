...
object BattleResultRouteContractTest {
  ...
  private def listParsesFiltersAndRendersRecords(): Unit = {
    ...
    withResultServer(service) { uri =>
      val response = get(uri.resolve("/api/battle/results?handle=Alice&battleId=battle-route&limit=2"))
  ...
  private def invalidHandleFilterShortCircuitsList(): Unit = {
    ...
    withResultServer(service) { uri =>
      val response = get(uri.resolve("/api/battle/results?handle=visitor"))
  ...
  private def recordPostParsesCommandAndValidationErrors(): Unit = {
    ...
    withResultServer(service) { uri =>
      val success = postJson(uri.resolve("/api/battle/results"), ValidRecordJson)
      val visitor = postJson(uri.resolve("/api/battle/results"), ValidRecordJson.replace("\"handle\":\"Alice\"", "\"handle\":\"visitor\""))
      val invalidBattle = postJson(uri.resolve("/api/battle/results"), ValidRecordJson.replace("\" battle-route \"", "\"  \""))
    ...
    withResultServer(failingService) { uri =>
      val serviceFailure = postJson(uri.resolve("/api/battle/results"), ValidRecordJson)
  ...
  private def withResultServer[A](service: RecordingBattleResultService)(run: URI => A): A = {
    ...
    server.createContext("/api/battle/results", exchange => routes.handle(exchange))
    server.start()
