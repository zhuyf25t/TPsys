...
import type {
  ReplayCommentApiRequestDto,
  ReplayCommentResponseDto,
  ReplayCommentsResponseDto,
  ReplayCommentWrapperResponseDto
} from "../../objects/replay/replayTypes";

export type ReplayCommentApiRecord = ReplayCommentResponseDto;

type ReplayCommentsResponse =
  Partial<Record<keyof ReplayCommentsResponseDto, unknown>> &
  Partial<Record<keyof ReplayCommentWrapperResponseDto, unknown>>;
...
export async function createReplayComment(input: {
  ...
  const controller = new AbortController();
  const timeout = window.setTimeout(() => controller.abort(), REQUEST_TIMEOUT_MS);
  const request: ReplayCommentApiRequestDto = {
    authorHandle,
    body
  };
