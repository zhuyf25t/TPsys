...
const REQUIRE_REMOTE_AUTH = CONFIGURED_AUTH_API_BASE.length > 0;
...
export function getCurrentAuthUser(): LocalAuthUser | null {
  ...
  if (!sessionToken || localCurrent?.sessionToken === sessionToken) {
...
export async function registerUser(input: {
  ...
  if (REQUIRE_REMOTE_AUTH) {
    ...
    return {
      ok: false,
      error: "后端注册失败：不会写入本地假账号，请检查后端和数据库。"
    };
  }
...
export async function loginUser(input: {
  ...
  if (isBuiltinAdminCredentials(input.handle, input.password)) {
  ...
  if (REQUIRE_REMOTE_AUTH) {
    ...
    return {
      ok: false,
      error: "后端登录失败：不会回退到本地账号，请检查账号或后端。"
    };
  }
...
async function tryRemoteRegister(input: {
  ...
  try {
    ...
    const payload = (await response.json()) as { handle?: string; skinId?: string; session?: string };
    const handle = payload.handle?.trim() ?? input.handle.trim();
    const user: LocalAuthUser = {
      handle,
      password: input.password.trim(),
      skinId: payload.skinId ?? getAuthSkinById(input.skinId).id,
      createdAt: Date.now(),
      sessionToken: payload.session ?? makeSessionToken(handle)
    };
...
async function tryRemoteLogin(input: {
  ...
  try {
    ...
    const payload = (await response.json()) as { handle?: string; skinId?: string; session?: string };
    const handle = payload.handle?.trim() ?? input.handle.trim();
    const existing = readUsers().find((user) => user.handle.toLowerCase() === handle.toLowerCase());

    const user: LocalAuthUser = {
      handle,
      password: input.password.trim(),
      skinId: payload.skinId ?? existing?.skinId ?? DEFAULT_SKIN_ID,
      createdAt: existing?.createdAt ?? Date.now(),
      sessionToken: payload.session ?? makeSessionToken(handle)
    };

    hydrateLocalSession(user, payload.session ?? user.sessionToken);
    recordBackendHealth(true);
    setCachedCurrentAuthUser(user);
    return { ok: true, user, sessionToken: payload.session ?? user.sessionToken };
  } catch {
    recordBackendHealth(false);
    return null;
  }
...
async function tryRemoteCurrentUser(sessionToken: string): Promise<LocalAuthUser | null> {
  try {
    ...
    try {
      ...
      const hydratedUser = {
        ...nextUser,
        skinId: payload.skinId ?? nextUser.skinId,
        sessionToken: payload.session ?? nextUser.sessionToken ?? sessionToken
      };
