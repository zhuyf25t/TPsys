...
import type { ProfileBattleHistoryItem, ProfileRatingHistoryPoint, ProfileSummary } from "../../../../apis/identity/profileGateway";
...
export function ProfilePageView({ resolvedHandle, profile }: ProfilePageViewProps) {
  if (!profile) {
    return (
          ...
          <main className="profile-cf__main">
            <section className="profile-cf__panel profile-cf__panel--chart" aria-label="评分曲线">
              <div className="panel-header panel-header--dense">
                ...
                <span className="panel-header__meta">{profile.ratingHistory.length > 0 ? `${profile.ratingHistory.length} 场` : "暂无足够对局"}</span>
              </div>
  ...
  return (
    <div className="profile-cf__chart-wrap">
      <svg className="profile-cf__chart" viewBox={`0 0 ${width} ${height}`} role="img" aria-label="评分历史图">
        ...
        <line className="profile-cf__chart-grid" x1={paddingX} x2={width - paddingX} y1={height - paddingBottom} y2={height - paddingBottom} />
