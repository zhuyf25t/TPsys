...
export interface ForumReplyApiRecord {
  id: string;
  author: string;
  body: string;
  publishedAt: number;
...
export interface ForumTopicApiRecord {
  id: string;
  title: string;
  author: string;
  excerpt: string;
  ...
  createdAt: number;
  body: string;
  replyItems: ForumReplyApiRecord[];
  viewerVote: "up" | "down" | null;
  score: number;
}
...
export async function createForumTopic(input: {
  ...
  return mutateForumTopic("/forum/topics", {
    title: input.title,
    body: input.body,
    tag: input.tag,
...
export async function createForumReply(
  ...
  return mutateForumTopic(`/forum/topics/${encodeURIComponent(normalizedTopicId)}/replies`, {
    body: input.body,
    author: input.author?.trim() || getCurrentAuthHandle()
  });
}
...
export async function setForumVote(
  ...
  return mutateForumTopic(`/forum/topics/${encodeURIComponent(normalizedTopicId)}/votes`, {
    author: input.author?.trim() || getCurrentAuthHandle(),
    vote: input.vote
  });
}
...
export async function setForumReplyVote(
  ...
  return mutateForumTopic(
    `/forum/topics/${encodeURIComponent(normalizedTopicId)}/replies/${encodeURIComponent(normalizedReplyId)}/votes`,
    {
      author: input.author?.trim() || getCurrentAuthHandle(),
      vote: input.vote
    }
...
function normalizeForumVote(value: unknown): "up" | "down" | null {
