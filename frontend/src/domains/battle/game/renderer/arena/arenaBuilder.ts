...
export function buildArena(context: ArenaBuilderContext): void {
  ...
  scene.cameras.main.setBackgroundColor(battleMap.themeId === "fall" ? "#243526" : "#0d0f0f");
  ...
  if (battleMap.themeId === "fall") {
    createFallMapObjects(context);
    return;
  }
...
function createBorderWalls(
  ...
  for (let x = FLOOR_TILE_SIZE / 2; x < WORLD_SIZE.x; x += FLOOR_TILE_SIZE) {
    ...
    }, { visible: getActiveBattleMap().themeId !== "fall" });
    createStaticObstacle(scene, wallBodies, obstacleBounds, occludables, {
...
function createFallMapObjects(context: ArenaBuilderContext): void {
  ...
  getActiveBattleMap().buildings.forEach((building) => {
    ...
    building.doors.forEach((door) => {
      scene.add
        ...
        .setDepth(48);
      scene.add.image(door.position.x, door.position.y, "fall-door").setDisplaySize(door.size.x, door.size.y).setDepth(49).setAlpha(0.72);
    });
...
function createStaticObstacle(
  ...
  if (getActiveBattleMap().themeId !== "fall" && obstacle.kind === "wall") {
