...
import services.battle.engine.BattleArenaCollision.*
import services.battle.engine.BattleGeometry.*
import services.battle.engine.BattleInitialLayout.*
import services.battle.engine.BattleInputRules.*
...
private[services] object BattleBotRules {
  ...
  private def aimAtTarget(player: BattlePlayerState, target: BotTarget, state: BattleAggregateState): BattleVector2 = {
    val lead = scale(target.player.movement, BattleBotCatalog.AimLeadDistance.value)
    ...
    val aimNoise = scale(perpendicular(normalizeAim(player.aim, base), wobbleDirection), BattleBotCatalog.AimErrorRadius.value)
  ...
  private def combatMovement(player: BattlePlayerState, target: BotTarget, state: BattleAggregateState): BattleVector2 = {
    ...
    val desired =
      if shouldRetreat(player, target) then coverOrRetreatDirection(player, target)
        ...
        val radial =
          if target.distance > BattleBotCatalog.PreferredRange.value + BattleBotCatalog.PreferredRangeAdvanceMargin.value then aim
          else if target.distance < BattleBotCatalog.PreferredRange.value - BattleBotCatalog.PreferredRangeRetreatMargin.value then scale(aim, -1.0)
  ...
  private def pickupObjective(player: BattlePlayerState, state: BattleAggregateState): Option[BattlePickupState] = {
    ...
    val needsHealth = player.hp.value <= player.maxHp.value * BattleBotCatalog.PickupHealthRatio
    val needsWeapon =
      currentWeapon(player).forall(_.weaponKind == WeaponKind.Pistol) ||
        currentWeapon(player).exists(weapon =>
          !weaponUsesHeat(weapon.weaponKind) &&
            weapon.ammoInMagazine.value <= math.max(1, math.floor(weapon.magazineSize.value * BattleBotCatalog.TacticalReloadRatio).toInt)
        )
    val preferred =
      ...
      else availablePickups.filter(pickup => distanceBetween(player.position, pickup.position) <= BattleBotCatalog.PickupSeekRange.value)
  ...
  private def shouldRetreat(player: BattlePlayerState, target: BotTarget): Boolean = {
    val lowHealth = player.hp.value <= player.maxHp.value * BattleBotCatalog.LowHealthRatio
    lowHealth || target.distance < BattleBotCatalog.PreferredRange.value - BattleBotCatalog.PreferredRangeRetreatMargin.value
  }
  ...
  private def coverOrRetreatDirection(player: BattlePlayerState, target: BotTarget): BattleVector2 = {
    ...
    candidates
      ...
      .maxByOption { direction =>
        val probe = clampToPlayable(add(player.position, scale(direction, BattleBotCatalog.CoverProbeDistance.value)))
      ...
      val best = candidates
        ...
        .map(direction =>
          val motion = findMotionDestination(
            ...
            direction = direction,
            distance = BattleBotCatalog.MovementProbeDistance.value,
  ...
  private def canBotFireAtTarget(state: BattleAggregateState): Boolean =
    state.elapsedMs.value >= BattleBotCatalog.OpeningFireDelay.value

  private def isBotFirePulseOpen(player: BattlePlayerState, state: BattleAggregateState): Boolean = {
    val interval = math.max(1L, BattleBotCatalog.FirePulseInterval.value)
    val window = math.max(1L, math.min(BattleBotCatalog.FirePulseWindow.value, interval))
