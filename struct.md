...
| typed DTO + Circe | `objects/apiTypes/*ApiTypes.scala`、`http4s/*Routes.scala` | 当前运行路径正在使用；battle result、bots、mail、social、forum response 已删除旧 route 字符串 render helper，只保留 DTO + Circe encoder 出口。 |
...
最高优先级不再是清理旧 route。当前旧 Java HttpServer 入口、旧 `HttpExchange` wrapper、旧 route contract tests、旧 battle 手写 JSON renderer，以及 battle result/bots/mail/social/forum response 的旧字符串 render helper 已清完；下一步应聚焦 service 简化或继续减少其他 `apiTypes` 中的手写 codec。
