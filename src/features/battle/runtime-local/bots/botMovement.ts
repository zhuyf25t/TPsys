...
import { normalizeVector } from "../geometry/sceneGeometry";
...
export function buildBotMovementVector(input: {
  ...
  if (input.target.kind === "pickup") {
    ...
    return ensureMeaningfulVector(
      normalizeVector({
        x: toTarget.x * (1.02 + input.tactic.supportWeight * 0.14) + awayFromEnemy.x * (0.72 + input.tactic.supportWeight * 0.2) + stuckVector.x * 0.2,
        y: toTarget.y * (1.02 + input.tactic.supportWeight * 0.14) + awayFromEnemy.y * (0.72 + input.tactic.supportWeight * 0.2) + stuckVector.y * 0.2
      }),
      escapedFallback
    );
  }
  ...
  if (input.target.distance > engageRange * 1.02) {
    return ensureMeaningfulVector(
      normalizeVector({
        x: toTarget.x * (1.04 + input.tactic.pursuitWeight * 0.18) + escapedFallback.x * 0.06 + stuckVector.x * 0.12,
        y: toTarget.y * (1.04 + input.tactic.pursuitWeight * 0.18) + escapedFallback.y * 0.06 + stuckVector.y * 0.12
      }),
      escapedFallback
    );
  }
  ...
  if (input.target.distance < engageRange * 0.58 || input.elapsedMs < input.brain.stuckUntilMs || input.tactic.enemyClose) {
    return ensureMeaningfulVector(
      normalizeVector({
        x: -toTarget.x * (0.64 + input.tactic.retreatWeight * 0.12) + strafe.x * (0.9 + input.tactic.strafeWeight * 0.14) + stuckVector.x * 0.24,
        y: -toTarget.y * (0.64 + input.tactic.retreatWeight * 0.12) + strafe.y * (0.9 + input.tactic.strafeWeight * 0.14) + stuckVector.y * 0.24
      }),
      escapedFallback
    );
  }
  ...
  return ensureMeaningfulVector(
    normalizeVector({
      x: toTarget.x * (0.36 + input.tactic.pursuitWeight * 0.14) + strafe.x * (0.88 + input.tactic.strafeWeight * 0.1) + stuckVector.x * 0.14,
      y: toTarget.y * (0.36 + input.tactic.pursuitWeight * 0.14) + strafe.y * (0.88 + input.tactic.strafeWeight * 0.1) + stuckVector.y * 0.14
    }),
    escapedFallback
  );
}
