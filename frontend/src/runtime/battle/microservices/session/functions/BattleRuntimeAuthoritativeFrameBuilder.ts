...
import type { BattleExtractionState, BattleGasZoneState, BattleLootCacheState
...
export function buildBattleRuntimeAuthoritativeFrame(
  ...
  return {
    ...
    gasZone: state.gasZone
      ? {
          ...state.gasZone,
          center: { x: state.gasZone.center.x, y: state.gasZone.center.y }
        }
      : null,
    extraction: state.extraction
      ? {
          zones: state.extraction.zones.map((zone) => ({ ...zone, position: { x: zone.position.x, y: zone.position.y } })),
          status: { ...state.extraction.status } as BattleExtractionState["status"]
        }
      : null,
    lootCaches: state.lootCaches.map((cache) => ({
      ...cache,
      position: { x: cache.position.x, y: cache.position.y },
      status: { ...cache.status } as BattleLootCacheState["status"]
    })),
