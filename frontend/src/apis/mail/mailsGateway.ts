...
import { backfillLocalBattleTruthToBackend, getMailEntries, markMailRead } from "../../runtime/battle/local/state/battleTruthStore";
import { getCurrentAuthSessionToken, getCurrentAuthUser } from "../identity/authGateway";
import type {
  MailFriendRequestStatusDto,
  MailItemResponseDto,
  MailKindDto,
  MailListResponseDto,
  MailReadApiRequestDto
} from "../../objects/mail/mailTypes";

export type FriendRequestMailStatus = MailFriendRequestStatusDto;
...
export async function loadRemoteMailSummaries(ownerHandle?: string | null): Promise<MailSummary[] | null> {
  ...
  try {
    ...
    try {
      ...
      const payload = (await response.json().catch(() => null)) as Partial<Record<keyof MailListResponseDto, unknown>> | null;
...
export async function markMailAsReadRemote(ownerHandle: string, mailId: string): Promise<boolean> {
  ...
  try {
    ...
    try {
      const request: MailReadApiRequestDto = {
...
type RemoteMailRecord = MailItemResponseDto;
