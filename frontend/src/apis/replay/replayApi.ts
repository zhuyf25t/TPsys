...
import type {
  ReplayCatalogItemResponseDto,
  ReplayCatalogResponseDto,
  ReplayDetailRecordResponseDto,
  ReplayDetailResponseDto,
  ReplayFrame
} from "../../objects/replay/replayTypes";
export { syncReplayToBackend, type ReplaySyncPayload } from "./replaySyncApi";
...
export type ReplayBackendCatalogItem = ReplayCatalogItemResponseDto;

export type ReplayBackendPlaybackItem = ReplayDetailRecordResponseDto;
...
export async function loadReplayCatalog(): Promise<ReplayBackendCatalogItem[] | null> {
  ...
  try {
    ...
    try {
      ...
      const payload = (await response.json()) as Partial<Record<keyof ReplayCatalogResponseDto, unknown>> | null;
...
export async function loadReplayPlayback(id: string, options: LoadReplayPlaybackOptions = {}): Promise<ReplayBackendPlaybackItem | null> {
  ...
  try {
    ...
    try {
      ...
      const payload = (await response.json()) as Partial<Record<keyof ReplayDetailResponseDto, unknown>> | null;
