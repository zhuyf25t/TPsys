...
import services.battle.microservices.queue.api.shared.{BattleQueueRequestDecodeError as BattleQueueStatusRequestDecodeError}
import services.battle.microservices.queue.objects.queue.{
  BattleQueueSnapshot,
  BattleQueueStatusQuery
}
import services.battle.microservices.queue.api.queue.BattleQueueStatusRequest.given
...
final case class BattleQueueStatusAPIMessage(
  ...
  private def readSnapshot(queueService: BattleQueueService, query: BattleQueueStatusQuery): IO[BattleQueueSnapshot] =
    for
      result <- queueService.status(query.ticketId)
