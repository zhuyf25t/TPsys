...
object PostgresSupport {
  ...
  private[database] def connectionResource(acquire: IO[Connection]): Resource[IO, Connection] =
  ...
  private[database] def buildHikariConfig(settings: PostgresConnectionSettings): HikariConfig =
  ...
  private[database] def withTransactionConnection[A](acquire: => Connection)(use: Connection => A): A = {
