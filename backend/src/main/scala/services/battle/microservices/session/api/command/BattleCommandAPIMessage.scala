...
object BattleCommandAPIMessage {
    ...
    Decoder.instance { cursor =>
      for
        request <- decodeRequest(cursor.value)
          .left
          .map(error => DecodingFailure(BattleCommandRequestDecodeError.message(error), cursor.history))
  ...
  private def decodeRequest(payload: io.circe.Json): Either[BattleCommandRequestDecodeError, BattleCommandRequest] =
