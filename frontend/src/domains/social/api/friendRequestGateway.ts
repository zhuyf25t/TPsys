...
import { getCurrentAuthSessionToken, getCurrentAuthUser, subscribeAuthState } from "../../identity/api/authGateway";
...
interface RemoteFriendRequestListResponse {
  requests?: unknown[];
}
...
function normalizeRemoteFriendRequestRecord(value: unknown): FriendRequestRecord | null {
  ...
  return {
    ...
    status,
    ...(typeof respondedAt === "number" ? { respondedAt } : {})
  };
}
