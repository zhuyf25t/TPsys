package slaydemo.backend.battle.objects.result
...
final case class BattleResultRecord(
  ...
  /**
   * 中文名：结果标识（resultId）。
   * 游戏视线：一场战斗会为每个玩家生成一条结算记录，这个 ID 用来稳定定位“某场 battle 下某个玩家的结算卡片”。
...
object BattleResultRecord {
  /**
   * 中文名：生成结果标识（resultId）。
   * 游戏视线：把战斗 ID 和玩家 handle 组合成唯一结算记录 ID，保证同一场战斗内每个玩家都有稳定可查的结果入口。
   * 建模原因：集中管理 ID 拼接规则，避免 route、service、frontend 各自拼接导致契约漂移。
   */
