...
import services.battle.objects.{
  BattleAPIRequestError as BattleQueueJoinRequestDecodeError,
  BattleQueueJoinCommand,
  BattleQueueSnapshot
}
import services.battle.objects.apiTypes.queue.BattleQueueJoinRequest.given
...
final case class BattleQueueJoinAPIMessage(
  ...
  private def authorizeJoin(
    ...
    for
      result <- IO.blocking(authorizationService.authorize(command))
  ...
  private def joinQueue(queueService: BattleQueueService, command: BattleQueueJoinCommand): IO[BattleQueueSnapshot] =
    IO.blocking(queueService.join(command))
}
