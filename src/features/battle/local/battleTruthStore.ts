...
export function finalizeBattleAndPersist(input: FinalizeBattleInput): FinalizeBattleOutput | null {
  const player = getPlayer(input.snapshot);
  if (!player) {
    return null;
  }
  ...
  const ratingBefore = getCurrentRatingFromState(state, activeHandle);
  const record = createBattleRecord(input.snapshot, player, input.finishedAt, ratingBefore, input.thumbnailDataUrl, activeHandle);
  const mails = createMailsForRecord(record);
  const playback = buildReplayPlayback(record, input.replayFrames);
  const storedReplayFrames = compactReplayFrames(input.replayFrames);

  const nextState: BattleTruthState = {
    version: 2,
    records: [record, ...state.records].slice(0, 50),
    mails: [...mails, ...state.mails].slice(0, 100)
  };

  saveLocalReplayPlayback(playback);
  writeState(nextState);
  markBattleTruthBackfillAttempt(record.id);
  void syncBattleResultToBackend({
    battleId: record.id,
    handle: activeHandle,
  ...
  void syncReplayToBackend({
    ...
    currentLoadout: record.currentLoadout,
    frameCount: storedReplayFrames.length,
    playbackAvailable: hasMeaningfulReplayFrames(storedReplayFrames),
    frames: storedReplayFrames
  });

  return {
    returnSummary: buildReturnSummary(record),
    replay: toReplayEntry(record)
  };
}
...
export function getMailEntries(): LocalMailEntry[] {
  const visibleMails = filterVisibleMails(readState().mails);
...
function createMailsForRecord(record: StoredBattleRecord): StoredBattleMail[] {
  ...
  const mails: StoredBattleMail[] = [
    {
      ...
      subject: "本局战报已送达",
      excerpt: "这场对局已经归档到战报库，你可以随时回看缩略图和结算摘要。",
      kind: "battle",
      important: true,
      unread: true,
      senderLabel: "战斗记录",
  ...
  if (record.ratingDelta !== 0) {
    mails.push({
      ...
      ownerHandle: record.handle,
      subject: "评分已更新",
      excerpt: `本局结算后，你的评分变化为 ${formatRatingDelta(record.ratingDelta)}。`,
      kind: "system",
      important: false,
      unread: true,
      senderLabel: "竞技中心",
...
function buildReturnSummary(record: StoredBattleRecord): LocalBattleReturnSummary {
  const mailCount = record.ratingDelta !== 0 ? 2 : 1;
...
async function runLocalBattleTruthBackfill(): Promise<void> {
  ...
  for (const record of state.records) {
    ...
    void syncReplayToBackend({
      ...
      finishedAt: record.finishedAt,
      finishedAtLabel: record.finishedAtLabel,
      title: `${record.resultLabel} 路 ${record.finishedAtLabel}`,
      modeLabel: "6 浜虹珵鎶€鍦?",
      resultLabel: record.resultLabel,
      mapLabel: record.mapLabel,
      highlightLine: record.highlightLine,
