...
object BattleResultRecordRequest {
  ...
  private def parseSubmissionHandle(
    ...
    if trimmed.isEmpty then Left(DecodingFailure(BattleResultRecordRequestDecodeError.message(BattleResultRecordRequestDecodeError.InvalidHandle), Nil))
    else if rejectVisitorLikeHandle && !HandlePolicy.isPlayableIdentityHandle(trimmed) then
      Left(DecodingFailure(BattleResultRecordRequestDecodeError.message(BattleResultRecordRequestDecodeError.VisitorNotAllowed), Nil))
  ...
  private def decodeRequest(cursor: HCursor): Either[DecodingFailure, BattleResultRecordCommand] =
    for
      ...
      requiredBattleId <- battleId.toRight(
        DecodingFailure(BattleResultRecordRequestDecodeError.message(BattleResultRecordRequestDecodeError.InvalidBattleId), cursor.history)
      )
