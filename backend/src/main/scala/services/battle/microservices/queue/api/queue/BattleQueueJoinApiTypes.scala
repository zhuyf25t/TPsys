...
import services.battle.microservices.queue.api.shared.BattleQueueRequestDecodeError
...
import services.battle.microservices.queue.objects.queue.{BattleQueueJoinCommand, QueueRequestId}
...
object BattleQueueJoinRequest {
  ...
  private def decodeHandle(cursor: HCursor): Either[DecodingFailure, PlayerHandle] =
    optionalText(cursor, "handle")
      ...
      .toRight(DecodingFailure(BattleQueueRequestDecodeError.message(BattleQueueRequestDecodeError.InvalidHandle), cursor.history))
  ...
  private def decodeSessionToken(cursor: HCursor): Either[DecodingFailure, SessionToken] =
    optionalText(cursor, "sessionToken")
      ...
      .toRight(DecodingFailure(BattleQueueRequestDecodeError.message(BattleQueueRequestDecodeError.MissingSession), cursor.history))
  ...
  private def invalidRating(cursor: HCursor): DecodingFailure =
    DecodingFailure(BattleQueueRequestDecodeError.message(BattleQueueRequestDecodeError.InvalidRating), cursor.history)

  private def invalidBattleMode(cursor: HCursor): DecodingFailure =
    DecodingFailure(BattleQueueRequestDecodeError.message(BattleQueueRequestDecodeError.InvalidBattleMode), cursor.history)
}
