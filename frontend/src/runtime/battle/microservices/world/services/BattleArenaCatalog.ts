...
import { NORMAL_BATTLE_MODE_LABEL } from "../../../../../objects/battle/objects/core/BattleModeDisplayLabels";
...
const FALL_HUNT_MAP_SPEC = fallHuntMapSpecJson as BattleMapSpecJson;
const WINTER_HUNT_MAP_SPEC = winterHuntMapSpecJson as BattleMapSpecJson;
const NORMAL_HUNT_MAP_SPEC = normalHuntMapSpecJson as BattleMapSpecJson;
...
function buildInnerObstacles(spec: BattleMapSpecJson): readonly ArenaObstacle[] {
  return [
...
function battlePickupDefinitionsFromMapSpec(spec: BattleMapSpecJson): readonly BattlePickupDefinition[] {
  return [
    ...spec.weaponPickups.map((pickup) => ({
      pickupId: pickup.pickupId,
      pickupKind: "Weapon" as const,
      weaponKind: pickup.weaponKind,
      position: copyVec(pickup.position)
    })),
    ...spec.itemPickups.map((pickup) => ({
      pickupId: pickup.pickupId,
      pickupKind: "Medkit" as const,
      weaponKind: null,
      position: copyVec(pickup.position)
    }))
  ];
...
function battlePickupDefinitionsFromLegacyConfig(
  weapons: ReadonlyArray<Readonly<WeaponPickupDefinition>>,
  items: ReadonlyArray<Readonly<ItemPickupDefinition>>
): readonly BattlePickupDefinition[] {
  return [
    ...weapons.map((pickup) => ({
      pickupId: pickup.pickupId,
      pickupKind: "Weapon" as const,
...
function fromMapSpec(spec: BattleMapSpecJson): BattleMapConfig {
  const spawnPoints = spec.heroDefinitions.map((definition) => copyVec(definition.position));
  const collisionObstacles = buildInnerObstacles(spec);
  return {
    mapId: spec.mapId,
    ...
    spawnPoints,
    collisionObstacles,
    pickupDefinitions: battlePickupDefinitionsFromMapSpec(spec),
    ...
    heroSpawnPoints: spawnPoints,
    innerObstacles: collisionObstacles,
    ...
    extractionZones: (spec.extractionZones ?? []).map((zone) => ({ ...zone, position: copyVec(zone.position) })),
    lootCaches: (spec.lootCaches ?? []).map((cache) => ({ ...cache, position: copyVec(cache.position) })),
    gasPlan: spec.gasPlan
      ? {
          center: copyVec(spec.gasPlan.center),
          stages: spec.gasPlan.stages.map((stage) => ({ ...stage }))
        }
      : null,
...
export const FALL_HUNT_BATTLE_MAP = fromMapSpec(FALL_HUNT_MAP_SPEC);
export const WINTER_HUNT_BATTLE_MAP = fromMapSpec(WINTER_HUNT_MAP_SPEC);
export const NORMAL_HUNT_BATTLE_MAP = fromMapSpec(NORMAL_HUNT_MAP_SPEC);
...
export const INDUSTRIAL_BATTLE_MAP = {
  ...
  spawnPoints: INDUSTRIAL_HERO_SPAWN_POINTS,
  collisionObstacles: INDUSTRIAL_INNER_OBSTACLES,
  pickupDefinitions: battlePickupDefinitionsFromLegacyConfig(
    INDUSTRIAL_WEAPON_PICKUP_DEFINITIONS,
    INDUSTRIAL_ITEM_PICKUP_DEFINITIONS
  ),
  sourceCrop: {
...
export function isNaturalBattleMapTheme(themeId: BattleMapThemeId): boolean {
  return themeId === "fall" || themeId === "winter" || themeId === "normal";
}
