...
export async function loadAndRegisterBotStrategyModule(
  moduleUrl: unknown,
  _options: BotStrategyModuleResolveOptions = {}
  ...
  return failure(
    "MODULE_LOAD_FAILED",
    `Dynamic bot strategy module loading is disabled for static frontend builds: ${trimmedModuleUrl}. ` +
      "Import the module statically and call registerBotStrategyFromModule(moduleNamespace, options)."
  );
}
