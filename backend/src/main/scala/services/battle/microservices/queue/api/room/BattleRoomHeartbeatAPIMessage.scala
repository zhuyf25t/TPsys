...
import services.battle.microservices.queue.api.shared.{BattleQueueRequestDecodeError as BattleRoomHeartbeatRequestDecodeError}
import services.battle.microservices.queue.objects.queue.{
  RealtimeRoomHeartbeatCommand,
  RealtimeRoomSnapshot
}
import services.battle.microservices.queue.api.room.BattleRoomHeartbeatRequest.given
...
final case class BattleRoomHeartbeatAPIMessage(
  ...
  private def heartbeat(queueService: BattleQueueService, command: RealtimeRoomHeartbeatCommand): IO[RealtimeRoomSnapshot] =
    for
      result <- queueService.heartbeat(command)
