...
import slaydemo.backend.replay.objects.ReplayTitle
...
private[services] object BattleFinishProjectionLabelRules {
  ...
  def highlightLine(
    ...
    BattleHighlightLine.fromWire(
      s"${BattleFinishProjectionPlayerRules.safeDisplayName(player)} 最终排名第 ${placement.value} 名，结算得分 $score，击杀 ${player.kills}，剩余生命 ${math.max(0, player.hp.value)}。"
    )
