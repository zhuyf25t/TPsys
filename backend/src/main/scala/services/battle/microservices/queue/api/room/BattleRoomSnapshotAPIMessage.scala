...
import services.battle.microservices.queue.services.{BattleQueueService, BattleRoomError}
import services.battle.microservices.queue.api.shared.{BattleQueueRequestDecodeError as BattleRoomSnapshotRequestDecodeError}
import services.battle.microservices.queue.objects.queue.{
  BattleRoomSnapshotQuery,
  RealtimeRoomSnapshot
}
import services.battle.microservices.queue.api.room.BattleRoomSnapshotRequest.given
...
final case class BattleRoomSnapshotAPIMessage(
  ...
  private def readSnapshot(queueService: BattleQueueService, query: BattleRoomSnapshotQuery): IO[RealtimeRoomSnapshot] =
    for
      result <- queueService.roomSnapshot(query.roomId)
