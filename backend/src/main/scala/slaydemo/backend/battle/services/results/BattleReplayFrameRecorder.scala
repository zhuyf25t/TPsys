...
private[services] object BattleReplayFrameRecorder {
  /** 中文名：更新frames（updateFrames）。游戏职责：在后端结算域中管理战报、回放、排名和历史记录，形成对局结束后的权威结果。 */
  def updateFrames(
  ...
  /** 中文名：append帧（appendFrame）。游戏职责：在后端结算域中管理战报、回放、排名和历史记录，形成对局结束后的权威结果。 */
  def appendFrame(
    frames: Vector[BattleReplayFrameState],
    elapsedMs: ElapsedMillis,
    players: Vector[BattlePlayerState],
  ...
  /** 中文名：capture帧（captureFrame）。游戏职责：在后端结算域中管理战报、回放、排名和历史记录，形成对局结束后的权威结果。 */
  def captureFrame(
    elapsedMs: ElapsedMillis,
    players: Vector[BattlePlayerState],
    projectiles: Vector[BattleProjectileState],
