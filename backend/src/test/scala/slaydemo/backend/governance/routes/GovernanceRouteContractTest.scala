...
object GovernanceRouteContractTest {
  def main(args: Array[String]): Unit = {
    ...
    println("Governance route contract checks passed")
  }
  ...
  private def contributionAdjustmentsListAndCreate(): Unit = {
    ...
    withGovernanceServer(contributionService, RecordingGovernanceNotificationService()) { uri =>
      val list = get(uri.resolve("/governance/contribution-adjustments?limit=3"))
      val created = postJson(
        uri.resolve("/governance/contribution-adjustments"),
  ...
  private def contributionAdjustmentValidationErrors(): Unit = {
    ...
    withGovernanceServer(contributionService, RecordingGovernanceNotificationService()) { uri =>
      val invalidActor = postJson(
        uri.resolve("/governance/contribution-adjustments"),
        """{"actorHandle":"moderator","targetHandle":"Alice","delta":5}"""
      )
      val invalidDelta = postJson(
        uri.resolve("/governance/contribution-adjustments"),
        """{"actorHandle":"admin","targetHandle":"Alice","delta":0}"""
      )
  ...
  private def adminNotificationsListAndCreate(): Unit = {
    ...
    withGovernanceServer(RecordingContributionAdjustmentService(), notificationService) { uri =>
      val list = get(uri.resolve("/governance/admin-notifications?kind=replay_report&targetType=replay&limit=4"))
      val invalidFilter = get(uri.resolve("/governance/admin-notifications?kind=appeal"))
      val created = postJson(
        uri.resolve("/governance/admin-notifications"),
  ...
  private def adminNotificationValidationErrors(): Unit = {
    ...
    withGovernanceServer(RecordingContributionAdjustmentService(), notificationService) { uri =>
      val invalidKind = postJson(
        uri.resolve("/governance/admin-notifications"),
        """{"kind":"appeal","targetType":"replay","targetId":"r1","body":"Review."}"""
      )
      val invalidBody = postJson(
        uri.resolve("/governance/admin-notifications"),
        """{"kind":"replay_report","targetType":"replay","targetId":"r1","body":" "}"""
      )
  ...
  private def withGovernanceServer[A](
    contributionService: RecordingContributionAdjustmentService,
    notificationService: RecordingGovernanceNotificationService
  )(run: URI => A): A = {
    val server = HttpServer.create(new InetSocketAddress("127.0.0.1", 0), 0)
    val routes = GovernanceRoutes(contributionService, notificationService)
    server.createContext("/governance/contribution-adjustments", exchange => routes.contributionAdjustments(exchange))
    server.createContext("/governance/admin-notifications", exchange => routes.adminNotifications(exchange))
    server.start()
