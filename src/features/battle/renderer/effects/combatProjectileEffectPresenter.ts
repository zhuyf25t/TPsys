...
import { getProjectileDisplayLabel } from "../../presenters/battleDisplayCatalog";
...
import { type CombatDebugReporter } from "../../debug/combatDebugReporter";
...
export function presentCombatProjectileEffect(input: {
  ...
  combatDebugReporter: CombatDebugReporter;
}): void {
  const { effect, snapshot, callbacks, combatDebugReporter } = input;
  ...
  if (effect.type === "no-damage") {
    combatDebugReporter.reportNoDamage(effect.reason);
    return;
  }
  ...
  combatDebugReporter.reportHit({
    projectileId: effect.projectileId,
    projectileKind: effect.projectileKind,
    projectileLabel: getProjectileDisplayLabel(effect.projectileKind),
