package services.battle.api.state
...
import services.battle.objects.apiTypes.state.BattleStateReadAPIRequest.given
...
final case class BattleStateReadAPIMessage(
  ...
  private def readState(stateService: BattleStateService, query: BattleStateReadQuery): IO[BattleAggregateState] =
    IO.blocking(stateService.currentState(query.battleId)).flatMap {
      case Right(state) =>
        IO.pure(state)
...
object BattleStateReadAPIMessage {
    ...
    Decoder.instance { cursor =>
      for
        request <- decodeRequest(cursor.value)
          .left
          .map(error => DecodingFailure(BattleStateReadRequestError.stateReadMessage(error), cursor.history))
  ...
  private def decodeRequest(payload: Json): Either[BattleStateReadRequestError, BattleStateReadQuery] =
    payload.as[BattleStateReadQuery].left.map(_ => BattleStateReadRequestError.InvalidJsonObject)
