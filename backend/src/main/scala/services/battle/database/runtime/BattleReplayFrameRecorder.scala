...
private[services] object BattleReplayFrameRecorder {
  /** 中文名：更新回放帧（updateFrames）。游戏职责：按事件、结束状态或采样间隔决定是否追加新的权威回放帧。 */
  def updateFrames(
  ...
  /** 中文名：追加回放帧（appendFrame）。游戏职责：替换同一时间点的旧帧并追加当前权威快照。 */
  def appendFrame(
    frames: Vector[BattleReplayFrameState],
    elapsedMs: ElapsedMillis,
    players: Vector[BattlePlayerState],
  ...
  /** 中文名：捕获回放帧（captureFrame）。游戏职责：把当前玩家、投射物和拾取物状态投影成可回放的帧数据。 */
  def captureFrame(
    elapsedMs: ElapsedMillis,
    players: Vector[BattlePlayerState],
    projectiles: Vector[BattleProjectileState],
